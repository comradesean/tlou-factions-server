...
| `t1/upgrades/upgrades.txt.crypt` | **yes, live** | `http://t1.final.prod.s3.amazonaws.com/t1/upgrades/upgrades.txt.crypt` | same `.crypt` container as campaign.config, a different key/value set (upgrade unlocks) | container format: `docs/protocol/userdata_and_campaign_config_crypt.md`; this file's own key set not separately documented yet |
| `t1/upgrades/extras.txt.crypt` | **yes, live** | `http://t1.final.prod.s3.amazonaws.com/t1/upgrades/extras.txt.crypt` | same container, extras/DLC-adjacent key set | same |
| `t1/upgrades/upgrades7.txt.crypt` | **yes, live** (CORRECTED - confirmed requested, an earlier pass here said otherwise) | `http://t1.final.prod.s3.amazonaws.com/t1/upgrades/upgrades7.txt.crypt` | presumably a build-01.11-specific variant of `upgrades.txt.crypt` (the `7` likely tracks the 01.11-era numbering seen elsewhere in this project, e.g. `net10.bin`) - key set not separately documented yet | - |
