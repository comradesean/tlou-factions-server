...
|---|---|---|---|
| `0x140 SetRoomFlags` `attr_selector` | client -> server | STRONGLY NARROWED 2026-08-19: live RPCS3 breakpoints at the sender (`0x00ad62dc`) hit three times across a private-match load-in - room creation, "Starting Game" loadout render, load-timer completion - all with the same value (1) and the same caller. A full-corpus search of every caller found only two literal values ever passed anywhere in the binary (1 and 0), both consistent with a single active/ready STATE FLAG rather than a selector choosing among several room attributes: 1 fires from a function whose first action is entering an active-phase setup sequence; 0 fires from a fire-once function named (by its working decompile filename) `lb_trigger` with heavy score/stat-style float math. No client branch reads the round-tripped value locally either way. STILL OPEN: no live capture has yet shown selector=0 on this project's own wire - only the call-site evidence supports it. | disasm `0x00ad6374`/`0x00ad82ec`/`0x00ad11fc` thunk/`0x0035a7dc`/`0x003f208c`; live RPCS3 debugger, 3 consecutive hits |
