...
| `room_flags_e8` / `room_flags_10` | - | GATE RESOLVED for `room_flags_e8` 2026-08-19: live RPCS3 breakpoints at its sender across two independent room types (party creation, game-room creation via find-match) both showed the gate register at 0 - the OR never fires in either sample, so the top-nibble variation already seen live must come from the raw `+0xe8` value itself, not this conditional. `room_flags_10` (`0x135`'s own, different function, "identical construction") likely follows the same pattern but wasn't independently live-checked. | disasm; live RPCS3 debugger for `room_flags_e8` |
| `member_data.rank_tier` | - | DC00 CONTAINER FORMAT SOLVED 2026-08-19 (`docs/protocol/dc_table.md`); the consuming function `FUN_003c8e30` is fully decompiled and the exact directory entry for hash `0xC85E199D` is pinpointed in net1.bin (file offset `0xed4` -> value blob at `0x27da8`, `{count=193, array->0x2950c}`). NAME CORRECTION 2026-08-19: `0xC85E199D` was reverse-matched against the retail disc's own DC source-symbol list (`crc32_mpeg2`, see `dc_table.md`) to `*net-money-info*`, not a rank/tier concept - the table is very likely a currency/scrap-economy table, which explains why `array`'s 193-entry payload never read cleanly as flat rank thresholds. Whether the wire field is genuinely money-derived or genuinely rank/tier-derived via a money-table lookup is still open. See `protos/common/member_data.ksy`'s `rank_tier` field doc. | DC00 container solved; backing DC symbol identified (`net-money-info`); array semantics and field semantics still open |
...
| `member_data.card_stat_2` / `card_stat_3` | - | Bytes and source pinned (`P+0x654`, a separate word preceding `custom_appearance`, corrected from an earlier "word 0 of that block" framing). EXHAUSTIVELY RE-CHECKED 2026-08-19: still exactly zero across all 842 live `0x13a` frames, even after the profile S3 round-trip was implemented and confirmed working with real non-zero data elsewhere in the same file - rules out "needs the round-trip" as the blocker. Display meaning remains unresolved. | disasm; live (842 frames) |
