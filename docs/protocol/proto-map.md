...
| `stat_line` task line's `%x` (`task-%x`) | client -> server | MECHANISM RESOLVED 2026-08-19 (corrects an earlier "shared connection/job-id" guess): a writer was found and traced (`FUN_0032241c`) - `param_1[0x1b]` is populated, alongside five sibling fields on the same struct, by a genuine DC-compiled-table lookup (base resolved via hash `0x1ad3445f`, per-field key lookup via `_opd_FUN_00ab685c`). This confirms the literal "task-" prefix's plain reading (a real task/objective id) rather than the generic job-id alternative. The specific task identity is DC-blocked, same as every other id->asset table in this project. | disasm `FUN_0032241c`, `research/ghidra/fm_applyrefs.txt`; DC-blocked for the specific id |
...
| DC-blocked set | All net-stat slots (including the supplies gate), `rank_tier` thresholds, and every id->asset map (cosmetics, character and name pools). PARTIALLY REOPENED 2026-08-19: `net1.bin`/`net10.bin` were assumed unreachable, but both are already decrypted and on disk (`server/data/served_content/*.psarc.crypt`, extractable with `server/lib/psarc_crypt.py`), and every DC hash this project has ever cited was found as a literal byte match inside them - the record format itself just isn't decoded yet. | See `research/notes/2026-08-19-dc00-record-format-handoff.md` for the extraction commands, confirmed hash hits, and the recommended next step (find the EBOOT's own DC00 parser rather than guessing the record stride from a hexdump). |
