...
| `member_data.rank_tier` | - | DC PATH RESOLVED 2026-08-20 (`research/notes/2026-08-20-dc-directory-and-catalogs.md` sections 1 and 3). The DC00 directory record is `{key_hash, type_hash, value_ptr}`, not `{value_ptr, key_hash, type_hash}` - the 2026-08-19 "193-entry array that never read as thresholds" was the PRECEDING record's table (`*net-emblem-layers-frame*`, the emblem shape catalog). `*net-money-info*`'s own value is `{count: 99, array -> dc1/net.bin 0x2665c}`, a clean monotonic cumulative ladder `0, 2000, 4000, 7000, 12000, ...`. Running `FUN_003c8e30`'s instruction-verified scan loop on it returns 0 on the first iteration (`array[0] == 0`, `array[1] > 0` trips the sentinel exit), so the DC branch is INERT: `rank_tier` is nonzero only via the override at `*(global+0x78)`, whose writer is untraced. Matches the capture - 0x0000 in 855/855 live `0x13a` frames. See `protos/common/member_data.ksy`'s `rank_tier` field doc. | DC branch fully solved and proven constant-0; only the `global+0x78` override's writer remains open |
