...
| `0x142 HostRank` `entries` | client -> server | STRUCTURE RESOLVED 2026-08-20, AND THE OLD FRAMING WAS WRONG. The producing getter is `FUN_003CD6C8`, slot 0 of the player vtable `0x01224438` (installed into all 8 player slots at `0x39c4ac`), and it returns a PACKED BITFIELD, not a rank: `entry = (b & 0xFFF) + (0x800 if a && (b & 0xFFF) else 0) + (c << 12)`, where `b = player+0x1A8`, `a = player+0x1B0`, `c = player+0x1AC` - all three written by `FUN_0039F75C` (`0x39fa34`/`0x39fa64`/`0x39fa6c`/`0x39fa90`), with `b`'s main source being `member_slot+0xE8`, itself the `0x131 Member` entry's own `member_id` off the wire (`lhz r0,0(r7)` @`0xad7854` -> `stw r9,232(r31)` @`0xad34f0`). So this was never blocked on a ranked account. Also proven: the same getter is used as a nonzero filter earlier in the loop (`0x39b7fc`), so an entry can never legitimately be 0. STILL OPEN: which input yields the live `0x0002` - the host is `member_id` 1 here yet no frame reports 1. Still proven NOT to be `member_data.rank_value`. | disasm `0x00ad60c4`, `0x003CD6C8`, `0x0039F75C`, `0x00AD34F0`; `server/logs/wire.jsonl` re-count (one frame carries `0x0002, 0x0003`, so the value is not a constant) |
