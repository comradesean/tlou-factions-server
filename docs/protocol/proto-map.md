...
| `stat_line` task line's `%x` (`task-%x`) | client -> server | MECHANISM RE-RESOLVED 2026-08-19 (corrects the prior day's own "task/objective-definitions table" reading, not just the original job-id guess): `FUN_0032241c` is a UI reward/notification-popup builder, not a task-table reader. Its DC base table for hash `0x1ad3445f` (located and structurally confirmed in `net1.bin`/`net10.bin` via `docs/protocol/dc_table.md`'s now-solved container format) is a **string-keyed HUD material/icon-path table** - `_opd_FUN_00ab685c` does a strcmp-based lookup, not a hash lookup, and the literal key resolved for `param_1[0x1b]` is the C string `"general/hud/prize-icon/Default"`. So `task-%x` is whatever integer that icon-path table associates with that fixed key, not a distinct per-task identity. UPDATE 2026-08-21/22: the resolved integer value is no longer unread - seven real captures now exist (`task-7d9d7acc`, `-ad611b77`, `-1ca07d2f`, `-8f436eab`, `-f72c7bf9`, `-a3dd87ca`, `-e4c65aa7`), all different. Live tracing found the fixed key is answered by a 5-slot dynamic content-module registry whose currently-registered module can change over a session, which is why a compile-time-constant key still yields a different resolved value call to call; none of the seven values themselves crack against this project's widest disc-symbol corpus. What's still open is narrower than before: which module the registry has registered at each point, and whether that correlates with chapter transitions - needs a live breakpoint test, not more static analysis. See `protos/0x11_stat_line.ksy`'s `%x` field doc, `research/notes/2026-08-21-task-hash-variation-trace.md`, `research/notes/2026-08-22-task-x-offset-reconciliation.md`. | disasm `FUN_0032241c`, `research/ghidra/fm_applyrefs.txt`, `docs/protocol/dc_table.md`; DC00 container solved, mechanism live-confirmed, registry-module identity still open |
| `member_data.card_string_0` / `card_string_1` | - | TYPE CORRECTED 2026-08-20: `P+0x654..0x65B` is not a numeric stat pair - it's an 8-byte, NUL-terminated ASCII string (`strcmp`/`strcpy` on the same buffer at `0xe459bc`/`0xe45b10`, confirmed instruction by instruction), and `card_string_0`/`card_string_1` are its first four characters. The buffer is reachable from exactly two functions in the whole binary, and the only value either can ever write into it (besides what it reads back from the profile) is the literal string `"*"`, gated on a net-event-recorder object's byte at `+0x40` that no store anywhere touches. Value space closed to `""` or `"*"` on 01.00 - consistent with 855/855 zero frames without "no live data ever populated it" as an unproven excuse. Display meaning of `"*"` (if it ever appears) is still open. See `research/notes/2026-08-20-tier2-followup.md` §2. | disasm `0x0034d378`, `0xe459bc`/`0xe45b10`; live (855 frames, all zero) |
| `value_20` / `value_22` / `value_pair_14` | - | PRODUCER RESOLVED 2026-08-20: `bl 0xacb6bc` @`0xad5c70`, a three-instruction getter reading floats off a config object at `+0x48`/`+0x4C` (`FUN_00acb6bc`). OBJECT IDENTITY RESOLVED 2026-08-21: that object is `g_70`/NetInfo (`0x013835c0`) - all three fields are one echoed per-client value, not independent coincidences. Semantic content (rating vs. location, two candidate leads found) still open - see `protos/0x12f_room_create.ksy`'s `value_20` doc. | disasm `0x00acb6bc`, `0x00ad5c68`, `0x00ad6cf4`; live register dump corroboration |
...
| `caller_arg_1c`, `is_party` (was `flag_27`, renamed 2026-08-21) | - | FULLY RESOLVED 2026-08-20 (closed same day, live). `caller_arg_1c` is the sender's `param_5` verbatim (constant `0xffff` across every known call site); `is_party` is `4` iff the sender's `param_4 != 0`, else `0`. All THREE `bctrl`-dispatched call sites are now known: `0x0035D440` (the `"Host"` game-room state, `r6=0`), `0x003B7FB0` (the `"GATHER"` game-room state, `r6=0`), and - found via a live RPCS3 breakpoint at `FUN_00ad5b78` during a real party join - `0x003CAC5C` inside `FUN_003CA9D0` (the 9-state room state machine), the PARTY path, with `r6=1`. That third site is exactly where the live `0x04` frames come from (`is_party = 4 iff r6 != 0`) - the exhaustive 3-for-3 party/non-party split across every known caller is why this field, unlike its still-hedged neighbors, was judged confident enough to rename. | disasm `0x00ad5bc0`-`0x00ad5cc0`, `0x0035D3F8`-`0x0035D440`, `0x003B7F3C`-`0x003B7FB0`, `0x003CAB84`-`0x003CAC5C`; live RPCS3 breakpoint for the party site; `research/notes/2026-08-20-followup-open-items.md` §1 |
...
| `single_player_server_hello` / `_response` / `stat_line` | client -> server | Structurally confirmed by the same shared-function argument as its siblings, and the post-hello payload grammar is fully resolved (`stat_line.ksy`). UPDATE 2026-08-21: the "0 of 452 captured hellos were this service" gap is closed - after a real dead upstream `campaign.config.txt.crypt` dependency was found and fixed (see `docs/protocol/userdata_and_campaign_config_crypt.md`), the service opened real connections and both line grammars were live-captured (two trophy lines, seven task lines), all matching the decompiled grammars byte-for-byte. | disasm `0x00080268`/`0x007f1acc`; both grammars LIVE-CONFIRMED 2026-08-21/22 |
...
|---|---|---|
| `report-server` response grammar | RESOLVED 2026-08-21, no longer open - was "request captured, reply never observed", but the grammar was fully derived from the binary rather than needing a retail capture: `+<ban_index> <name>`, single-pass, fail-open on every branch, re-verified instruction-by-instruction against a fresh 01.11 objdump. See `protos/0x11_report_line.ksy`'s `ban_reply_row` type and `research/notes/2026-08-21-report-server-response-grammar.md`. | Closed. **Fail-open re-verified 2026-08-19 against the 01.11 ELF** (the earlier citation pointed into `research/disasm/full.asm`, which is the **01.00** binary - report-server is 01.11-only, so that address was an unrelated instruction): `-1` default stored @`0x36e1a0`/`0x36e1a8`, the `'+'` test `cmpwi cr7,r0,43` @`0x36e2cc` skipping to the shared done-label `0x36e388`, and `+916`/`+920` written only on a strcmp match @`0x36e360`-`0x36e368`. Nothing needs doing unless a deliberate ban is ever wanted - tagged `TODO(pReportArray)` / `TODO(g_net+920)`. `pReportArray`'s DC directory entry (hash `0xFFAC56F2`) is now pinpointed in net10.bin (`server/ticket_server.py`'s TODO comment) - it currently defines exactly one entry; the entry's name/id are the remaining unknowns (data-compiler-blocked, not needed to answer the ban check correctly). |
| `profile_21` zero region `P+0x1E74..0x5008` | MOSTLY RESOLVED 2026-08-21 (see `docs/protocol/profile_21_record.md` and `research/notes/2026-08-21-profile21-zero-region-walk.md`) - `P+0x1E8C..0x5008` is CLOSED (confirmed zero, reserved/unused); `P+0x1E74..0x1E8B` (`promotion_flags_1e74`, six BE u32 words) is narrowed but not closed - non-zero on one of three accounts, correlates with `milestone_latch_1e2c`, static analysis now exhausted (DLC-entitlement and capability-register hypotheses both tested and ruled out; a strong circumstantial `promo1/` `.edat` lead exists but is unproven). | Only remaining unblock: a live RPCS3 memory-write-breakpoint test, plan in the zero-region-walk note's §5. |
...
| `stat_line` task line's 2nd `%s` | RESOLVED (live) 2026-08-21, no longer open. The accessor was pinned (`base+0x2e6c` string field, `_opd_FUN_00952520`), but `base` resolved through a double pointer indirection to `0x01441194` - outside the static file image - so it needed a live read: a real captured task line resolved it to `BCUS98174`, the title's own PS3 product/serial code (a per-title runtime constant, not save-specific). See `protos/0x11_stat_line.ksy`. | Closed - live-captured. |
