...
| `0x136 attr_tail` (20 bytes) | The trailing 20 bytes of the 36-byte attribute block at `0x14:0x38`, whose first 16 are the live-confirmed `host_npid`. Zeros work and matchmaking succeeds - but that is absence of evidence, not proof of inertness: a consumer keyed off it would show as a cosmetic or filtering difference, not a hard failure. | Decompile the copy on both sides - the `0x136` case of `FUN_00ad7604` (byte-copies from `wire[0x14]`) and `_opd_FUN_003b2a9c` - and correlate against a capture varying one search option at a time. |
