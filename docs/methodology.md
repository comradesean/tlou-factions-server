...
Live capture (step 1's "or from an observed byte pattern in a live capture" path, and step 3's "once available" capture cross-reference) is no longer a future unlock either — it's the active daily workflow. A self-hosted RPCN fork plus custom stub servers (`tools/*_stub.py`) get a real RPCS3 client through auth, lobby, room hosting, and into an actual loaded match, which is how most of the `NetMatchmaking*` family's live-tested behavior (step 3) got confirmed.
