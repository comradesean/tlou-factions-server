...
seq:
  ...
  - id: server_choice_1
    size: 3
    doc: "Offset 1:4. UNCONSTRAINED server-choice bytes. RESOLVED 2026-08-18: the client's receive path (FUN_00acc424) reads the 8-byte reply via _opd_FUN_00acbd98 and the ONLY bytes it acts on are byte[0] (the 0x22 magic check) and bytes[4:8] (stored to conn+0x50 as the framing nonce). Bytes [1:4] are never read or branched on anywhere - so the client imposes no structure and a capture could not 'define' them (they are whatever a server chooses to send). A stub may send zero. (Was `unknown1`, flagged as possibly needing a capture; the client-code trace shows no capture is needed.)"
  - id: session_token
    type: u4
