...
**Backend base decided:** use RPCN as-is for the generic PSN transport layer (room hosting via `sceNpMatching2`, NAT traversal via `sceNpSignaling`, base auth/tickets) - not forking it. On top of that, build custom Factions-specific backend services for what RPCN doesn't and can't know about:

- **Character customization / persistent profile data** - evidence: `game/net/net-clan-manager.cpp`, `net-booster-manager.cpp`, `net-buff-manager.cpp`, `in-game-commerce.cpp`, plus the `cellUserInfo` and `sceNpCommerce2` imports (see `docs/tooling.md` for the full import list).
- **"Experience"/progression** - evidence: `game/net/net-tus-variable.cpp`, almost certainly backed by Sony's Title User Storage (`sceNpTus`, part of the `sceNp2` import block, 29 functions) - the standard PS3 mechanism for persistent per-player stats/XP.
- **Lobby/matchmaking rules specific to Factions** - RPCN hosts rooms mechanically via `sceNpMatching2`, but the actual matchmaking criteria, ready-up/team-assignment flow, and room search/attribute data are game-specific and live in `game/net/lobby-flow.cpp` / `net-matchmaking.cpp` / `net-menu-host.cpp` and the `NET_SM_*` state catalog (`protos/pending/net_sm_states_catalog.md`) - need to be understood well enough to replicate whatever the client reads/writes through room attributes.

This is the concrete backend scope going forward: RPCN (unmodified) + a Factions-specific service covering profile/customization, TUS-backed progression, and lobby/matchmaking logic.
