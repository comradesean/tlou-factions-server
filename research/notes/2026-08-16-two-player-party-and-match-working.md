...
**Status: confirmed live, 2026-08-16, same marathon session as the solo-host
...
- Accepting the invite joins the party: the joiner's client sends **`0x130`
  RoomJoin** (88 bytes — first live captures this session: offset 8 = the
...
1. **Remote player's rank never renders** (either client). Find-match
   ...
   servers). Delegated to a Ghidra/objdump agent (in flight as of this
   note): trace 0x142's sender + the rank UI's data source.
2. **Remote player's gear/customization randomized.** Possibly the same
   per-member data channel; same agent is tracing it. Next live experiment:
...
4. One host-side RPCS3 crash mid-session (suspected emulator bug, not
   reproduced).
