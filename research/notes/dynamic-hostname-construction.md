...
- `http://%s.naughtydog.com` (offset `0xe6a248`) - the format template. **This one actually was found in the initial strings pass** (listed under "urls/schemes" in the original findings) - the miss was not recognizing what fills the `%s`, not failing to find the string at all.
...
## Other `%s` URL templates found (follow-up pass, same day)
...
- `http://%s.s3.amazonaws.com` and `http://%s.s3.amazonaws.com/t1/upgrades/%s` - likely substitution values already known as *complete* literal strings from the initial pass (not templated themselves): `t1.patch`, `t1.campaign.config` (full hostnames `t1.patch.s3.amazonaws.com` / `t1.campaign.config.s3.amazonaws.com` were already found whole in `research/notes/static-recon-findings.md`).
